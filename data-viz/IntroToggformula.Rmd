...
7) Create a scatterplot using `YearBuilt` as the explanatory variable and `SalePrice` as the response variable. Include a regression line, a title, and labels for the x and y axes.
...
11) Pick another quantitative variable from the  `AmesHousing2` data set and describe its distribution using the `gf_histogram()` function. Use a secondary variable to see more detailed patterns, such as `fill = ~KitchenQual`. Consider different bin widths to see if the shape of the distribution changes. Type `?gf_histogram` in the command line and then learn how to use the `bin` and `binwidth` arguement. 
