...
**Data**: We will use a dataset based upon the [Global Terrorism Database](https://www.start.umd.edu/gtd/) which consists of all terrorist incidents from 1970-2013. This is a large file and may take a while to load.
We will also use a shapefile of the world's state boundaries that was downloaded from the [Natural Earth website](https://www.naturalearthdata.com/downloads/50m-cultural-vectors/50m-admin-0-countries-2/).

	
**Note**: You must download the "ne_50m_admin_0_countries" folder and update the file path to indicate its location for this code to run:
...
Worldshapes <- readOGR(dsn = "C:/Users/Ryan M/Documents/Grinnell/ne_50m_admin_0_countries", layer = "ne_50m_admin_0_countries")
