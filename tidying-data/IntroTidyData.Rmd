...
This data set started in a relatively tidy form, so it may be difficult to see the benefit of tidying it. Tidy data are typically required for summarizing and plotting data in R. For example, consider making a side-by-side boxplot using `ggplot2` (we learn more about plotting data in the ggplot and ggformula tutorials).
